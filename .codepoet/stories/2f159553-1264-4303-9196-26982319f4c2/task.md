...
# Your Task: Three pages display with proper layout, typography, and visual hierarchy ready for content

## Description
Create the three main portfolio pages with proper layout structure, placeholder content areas, and comic-styled typography. This phase focuses on layout and visual hierarchy rather than final content, establishing the template for each page type.

## Acceptance Criteria
- Home page displays hero section, featured projects grid, and intro section with proper layout
- About page displays profile section, skills grid, and bio sections with consistent spacing
- Experience page displays timeline of roles with dates, companies, and descriptions
- All pages use reusable components (ProjectCard, SkillBadge, TimelineItem) consistently
- Typography is applied with ink effects and scales correctly across breakpoints
- Placeholder content areas are clearly marked and ready for final content

## Implementation Notes
- Create typography.css with font-family stack (serif for headings, sans-serif for body), font-sizes scaled to breakpoints, line-height 1.6 for readability, apply ink effects via text-shadow
- Build Home page with hero section (large heading, subheading, CTA button), featured projects grid using Grid component, and brief intro section
- Build About page with profile section (placeholder image area with comic border), skills grid using SkillBadge components, and bio text sections
- Build Experience page with timeline layout using TimelineItem components, each showing role, company, dates, and description placeholder
- Create ProjectCard component with image placeholder, title, description, and tech tags with accent color underlines
- Create SkillBadge component displaying skill name with optional accent color border, used in grid layout
- Create TimelineItem component with vertical line connector, date badge, and content area for experience entries

## Completion
Verify your changes work — run relevant tests or checks appropriate for this project.

Then create `.codepoet/stories/2f159553-1264-4303-9196-26982319f4c2/done.json` with this exact structure:
```json
{
  "status": "completed",
...
```
IMPORTANT: The file MUST be at exactly `.codepoet/stories/2f159553-1264-4303-9196-26982319f4c2/done.json`.
